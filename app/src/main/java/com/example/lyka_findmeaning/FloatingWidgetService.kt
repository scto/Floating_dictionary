...
class FloatingWidgetService : Service() {
    ...
    private val respository by lazy { Getmeaningrepository() }
    ...
    override fun onCreate() {
        ...
        searchbtn.setOnClickListener {
            GlobalScope.launch { _meaning.emit(Resource.Loading()) }

            val word = (inputfield as EditText?)?.text.toString().trim()
            GlobalScope.launch(Dispatchers.IO) {
                val res =
                    respository.getMeaning(
                        word,
                        object : Getmeaningcallback {
                            override fun onSuccess(word: word?) {

                                GlobalScope.launch { _meaning.emit(Resource.Success(word!!)) }
                            }

                            override fun onError(errorMessage: String) {
                                GlobalScope.launch { _meaning.emit(Resource.Error(errorMessage)) }
                            }
                        },
                    )
            }
    ...
    private fun emptyText() {
        ...
        mainHandler.post { tv_meaning.visibility = View.GONE }
    }
    ...
    private fun stopLoading() {
        ...
        val mainHandler = Handler(Looper.getMainLooper())
        mainHandler.post { searchbtn.revertAnimation() }
    }

    private fun showLoading() {

        val mainHandler = Handler(Looper.getMainLooper())
        mainHandler.post { searchbtn.startAnimation() }
    }
